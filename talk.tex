...
\begin{frame}
	...
	{\color{magenta}\det{|\hat Q_h|{\cal R}(\hat Q_h)}}\\
		...
		e^{-S_{PF}}}_{\text{CLOVERDET} } \underbrace{\color{red} e^{-S_{PF}'}}_{\text{CLOERDETRATIO}}...\,
			\only<-1>{e^{-S_{det}^h}}
			\only<2->{\tikz[anchor=base, baseline] \node[fill=magenta!40,rounded corners=2pt] (d1) {$e^{-S_{det}^h}$}; }
		...
		\path[->]  (10,1) edge [in=190, out=270]  (15,0.5);
		\end{tikzpicture}}
		\item<3-> Molecular dynamics
		\begin{align*}
			\partial_t U&=\Pi\\
...
%			\end{hpcablock}
%		}
		\item<4-> Inversion of $\hat Q_\pm$, $\hat W_{\pm}$, $\hat Q_h$ on QUDA 
		\item<5-> $\delta S_{gauge}$ on QUDA
		\item<6-> Porting $\delta S_{det}^{(h)}$, $\delta S_{PF}^{(h)},...$ on QUDA in progress.
	\end{itemize}
