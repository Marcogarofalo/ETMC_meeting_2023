...
\begin{itemize}
	...
	\begin{flalign*}
		Z= \int DU D\psi D\bar\psi \,e^{-S_{gauge}-\bar \psi M_{deg}\psi - \bar \Psi M_{deg} \Psi }&&
	\end{flalign*}
	...
	\uncover<4->{\begin{hpcablock}{rational approximation partial fractions}
		...
		\begin{minipage}{\textwidth}
			\begin{columns}
				...
				\begin{itemize}
					\item $N \approx 10$, with $\mathcal{R}$ split across 2-3 monomials $r$ on 2-3 timescales (usually 3)
...
\begin{frame}
	\begin{itemize}
		\item Determinant evaluated with pseudofermions, diagonal operetor as $\det M_{ee}=\exp{\tr[\log M_{ee}]}$
		\begin{flalign*}
	...
	{\color{magenta}\det{|\hat Q_h|{\cal R}(\hat Q_h)}}\\
		...
		e^{-S_{det}^h}
		\underbrace{\color{mygreen} e^{-S_{PF}^h}}_{\text{NDCLOVERRATCOR}}e^{-S_{PF}'^h}...\, \underbrace{ \color{magenta} e^{-S_{corr}}}_{\text{NDCLOVERRATCOR}}
		\end{flalign*}
		...
		\end{align*}
		\item Application and inversion of $\hat Q_\pm$ and $\hat Q_h$ on GPU with QUDA
