...
\author[B.~Kostrzewa]{Bartosz Kostrzewa, M.~Garofalo, S.~Romiti \\ S.~Bacchio, J.~Finkenrath, F.~Pittler}
\institute[HPC/A-Lab, Bonn~U.]{High Performance Computing \& Analytics Lab,\\ Rheinische Friedrich-Wilhelms-Universit{\"a}t Bonn}
\date[August 8th, LAT'22, Bonn]{August 8th 2022, 39th International Symposium on Lattice Field Theory, Bonn}
...
\begin{frame}{The cost of ensemble generation (at phys. $M_\pi$ on CPU machines)}
  \centering
  \includegraphics[height=65mm,clip,trim = 46mm 0 0 0]{ensembles_asquared_mpi}
  \includegraphics[height=65mm]{ensembles_phys_point}
  \,\,\,\,\,
...
\begin{itemize}
	...
	\begin{flalign*}
		Z= \int DU D\psi D\bar\psi \,e^{-S_{gauge}-\bar \psi D_{deg}\psi - \bar \Psi D_{deg} \Psi }&&
	\end{flalign*}
	\item Integrating fermion variables
	\begin{flalign*}
		Z= \int DU  \,e^{-S_{gauge} }\det{(D_{deg})}\det{(D_{ndeg})}&&
	\end{flalign*}
	...
	\begin{flalign*}
		...
		\det{(Q^h)}&&
	\end{flalign*}
	\item {\color{red} Hasenbusch trick}
	\begin{flalign*}
	Z= \int DU  \,e^{-S_{gauge} }{\color{red}\det{\left(M_{ee}^+M_{ee}^-\right)}
	\det{(W_{+}W_{-})}	\det{(W_{+}^{-1}Q_+Q_-W_{-}^{-1})}...\,}
	\det{(M_{ee}^{h})}
	\det{(Q^h)}&&
	\end{flalign*}
	\item {\color{blue} Rational HMC and correction}
	\begin{multline*}
	Z= \int DU  \,e^{-S_{gauge} }\det{\left(M_{ee}^+M_{ee}^-\right)}
	\det{(W_{+}W_{-})}	\det{(W_{+}^{-1}Q_+Q_-W_{-}^{-1})}...\,\\
	\det{(M_{ee}^{h})}
	{\color{blue}\det{(  r_1(Q^h) )}\det{(  r_2(Q^h) )}...\,\det{(|Q^h|{\cal R}(Q^h)}}
	\end{multline*}
\end{itemize}	
\end{frame}
...
\begin{frame}
	\begin{itemize}
		\item Determinant evaluated with pseudofermions
		\begin{flalign*}
		Z&= \int DU  \,e^{-S_{gauge} }\det{\left(M_{ee}^+M_{ee}^-\right)}
		\det{(W_{+}W_{-})}	\det{(W_{+}^{-1}Q_+Q_-W_{-}^{-1})}...\\
		&\hspace{7cm}\det{(M_{ee}^{h})}
		\det{(  r_1(Q^h) )}\det{(  r_2(Q^h) )}...\,\det{|Q^h|{\cal R}(Q^h)}\\
		&= \int DU  \,e^{-S_{gauge} } \underbrace{e^{-S_{det}}
		e^{-S_{PF}}}_{\text{CLOVERDET} } \underbrace{e^{-S_{PF}'}}_{\text{CLOERDETRATIO}}...\,
		e^{-S_{det}^h}
		e^{-S_{PF}^h}e^{-S_{PF}'^h}...\, e^{-S_{corr}}
		\end{flalign*}
		\item Molecular dynamics
		\begin{align*}
			\partial_t U&=\Pi\\
			\partial_t \Pi&=\delta S_{gauge}+\delta S_{det}+\delta S_{PF}+\delta S_{PF}'+...+\delta S^h_{det}+\delta S^h_{PF}+\delta S'^h_{PF}+...+\delta S_{corr}
		\end{align*}
		\item Application of $D^{-1}$ inside the forces computed on the GPU with QUDA
		\item $\delta S_{gauge}$ on QUDA
		\item Porting $\delta S_{det}^{(h)}$, $\delta S_{PF}^{(h)}$ on QUDA in progress.
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Current state of the port}
  \begin{columns}
    \column{0.45\textwidth}
      \centering
    ...
    \column{0.55\textwidth}
      \begin{tabular}{cccc}
        machine                  & real time  & node-hours (CPU) / &    kWh      \\
                                 &            & GPU-hours          &             \\
        \hline \\
        64 nodes         &  $2.61$ h  & 167                        &    $\sim 84$      \\
        (Juwels)         &            &                            &             \\
                         &            &                            &             \\
        32 GPUs          &  $1.58$ h  & 50.6                       &    $\sim 24$    \\    
        (Juwels Booster) &            &                            &                
      \end{tabular}
      \vspace{0.3cm}
      \begin{itemize}
        \item CPU strong scaling to 64 nodes okay, not great beyond that $\rightarrow$ real throughput limitation
        \vspace{0.2cm}
        \item gets (much) worse for larger volumes where many more nodes are required (depends on machine though) 
        \vspace{0.2cm}
        \item Improvement factor CPU/GPU in energy usage already $\sim 3.5$
        \vspace{0.2cm}
        \item Expect another factor of $2$ to $2.5$
        \vspace{0.2cm}
        \item Finally we will be able to run a trajectory in less than one hour again!
      \end{itemize}
  \end{columns}
\end{frame}
  ...
  \begin{columns}
    \column{0.50\textwidth}
      \begin{hpcablock}{$64^3 \cdot 128$ @ $M_\pi \sim 135$ MeV}
        \includegraphics[width=0.95\textwidth, page=1]{Juwels_Booster_HMC_Scaling}
      \end{hpcablock}
    \column{0.50\textwidth}
      \begin{hpcablock}{$112^3 \cdot 224$ @ $M_\pi \sim 135$ MeV}
        \includegraphics[width=0.95\textwidth, page=2]{Juwels_Booster_HMC_Scaling}
      \end{hpcablock}
  \end{columns}
  \vspace{0.2cm}
  \begin{itemize}
